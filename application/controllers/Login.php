...
class Login extends CI_Controller {
        ...
        if($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                if($validity === 'unverified_user'){
                    $this->session->set_flashdata('info_message', get_phrase('you_have_already_registered').'. '.get_phrase('please_verify_your_email'));
                }else{
    ...
    public function verify_email_address() {
        ...
        if($user_details->num_rows() > 0) {        
            ...
            $this->session->set_flashdata('error', get_phrase('the_verification_code_is_wrong').'.');
        }
    }
}
